...
router.post('/joy', (req, res) => {
    pool.query('SELECT no, title, nickname, content, created_date FROM communityjoy', (error, results) => {
      if (error) {
        console.error(error);
...
router.get('/joy/delete/:no', (req, res) => {
    ...
    pool.query('SELECT nickname FROM communityjoy WHERE no = ?', [paramsNo], (error, results) => {
      if (error) {
  ...
  router.get('/joy/update/:no', (req, res) => {
    ...
    pool.query('select * from communityjoy where no = ?', [paramsNo], (error, results) => {
      if (error) {
        console.error('쿼리 실행 중 오류 발생: ', error);
        res.status(500).send('내부 서버 오류');
      } else {
