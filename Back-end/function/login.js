...
router.post('/process/login', (req, res) => {
  ...
  pool.getConnection((err, conn) => {
    ...
    conn.query('SELECT id, nickname, password, roomid FROM users WHERE id = ?', [paramID], async (err, rows) => {
      conn.release();
      if (err) {
        console.dir(err);
      ...
      if (rows.length > 0) {
        ...
        if (match) {
          console.log('아이디 [%s], 패스워드가 일치하는 이름 [%s] 찾음', paramID, user.nickname);
          req.session.user = {
            id: user.id,
  ...
  try {
    ...
    pool.getConnection((err, conn) => {
      ...
      const exec = conn.query(
        ...
        [paramLastName, paramFirstName, paramGender, parambirth, paramNickname, paramID, hashedPassword, paramState],
        (err, result) => {
          conn.release();
